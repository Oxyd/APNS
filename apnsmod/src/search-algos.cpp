...
boost::optional<piece::color_t> winner(board const& board, piece::color_t player) {
  ...
  position::row_t const players_target_row    = player == piece::gold ? board::MAX_ROW : board::MIN_ROW;
  position::row_t const opponents_target_row  = player == piece::gold ? board::MIN_ROW : board::MAX_ROW;

  bool player_has_rabbits = false;
  bool opponent_has_rabbits = false;

  for (position::col_t col = board::MIN_COLUMN; col <= board::MAX_COLUMN; ++col) {
    boost::optional<piece> const piece = board.get(position(players_target_row, col));
  ...
  for (position::col_t col = board::MIN_COLUMN; col <= board::MAX_COLUMN; ++col) {
    boost::optional<piece> const piece = board.get(position(opponents_target_row, col));
    if (piece && piece->type() == piece::rabbit) {
  ...
  if (!(player_has_rabbits && opponent_has_rabbits)) {
    ...
    for (position::row_t row = board::MIN_ROW + 1; row <= board::MAX_ROW - 1; ++row) {  // We've already cheched first and last row.
      for (position::col_t col = board::MIN_COLUMN; col <= board::MAX_COLUMN; ++col) {
...
void unapply(move_path const& path, board& board) {
  for (move_path::const_reverse_iterator v = path.rbegin(); v != path.rend(); ++v) {
    if (*v && (**v).step) {
...
boost::optional<move_path> player_moves::query(zobrist_hasher::hash_t hash, board const& expected_position) {
...
boost::shared_ptr<player_moves> moves::get(zobrist_hasher::hash_t root_hash, board const& root_pos,
                                           vertex* root, zobrist_hasher const& hasher, piece::color_t attacker) {
  ++time_;

  records_map::iterator record = records_.find(root_hash);
  if (record != records_.end() && record->second.moves && record->second.moves->root_position() == root_pos) {
    ++hits_;
...
void moves::gather(boost::shared_ptr<player_moves> const& moves, search_stack& stack) {
  ...
  for (vertex::children_iterator child = current->children_begin(); child != current->children_end(); ++child) {
    ...
    if (child->type != stack.path().front()->type) {
      // This is a move terminal. Insert the child into moves and do not recurse into it.
      move_path child_path = {{}};
      assert(stack.path().size() <= 5);
      std::copy(stack.path().begin() + 1, stack.path().end(), child_path.begin());
...
void moves::resize_records(std::size_t new_size) {
  while (records_.size() > new_size) {
    records_map::iterator to_remove = std::min_element(records_.begin(), records_.end(),
...
bool moves::records_compare(records_map::value_type const& lhs, records_map::value_type const& rhs) {
...
void search_tree::evaluate() {
  ...
  if (child.proof_number == 0 && child.disproof_number == 0) {
    ...
    // Only check for win if this is the start of a move and if a rabbit was moved or captured in this move.
    if (child.type != parent->type) {
      search_stack::path_sequence::const_reverse_iterator iter = stack_.path().rbegin();
...
void search_tree::cut_children() {
  ...
  // Simply remove the whole move from the DB. It will be regenerated later on.
...
void search_tree::update_path() {
  ...
  search_stack::hashes_sequence::const_reverse_iterator hashes_current    = stack_.hashes().rbegin();
  ...
  while (true) {
    ...
    if (proof_tbl_ && (current.proof_number == 0 || current.disproof_number == 0)) {
      ...
      std::copy(history_current, stack_.history().rend(), entry.history.rbegin());
      proof_tbl_->insert(*hashes_current, ply, entry);
    }

    if (trans_tbl_ && (current.proof_number != 0 && current.disproof_number != 0)) {
      trans_tbl_->insert(*hashes_current, ply, std::make_pair(current.proof_number, current.disproof_number));
    }

    search_stack::path_sequence::const_reverse_iterator parent = boost::next(path_current);
...
void search_tree::find_move_root(vertex*& root, zobrist_hasher::hash_t& root_hash, board& position,
                                 std::size_t& move_len) {
  search_stack::path_sequence::const_reverse_iterator   current       = stack_.path().rbegin();
  search_stack::hashes_sequence::const_reverse_iterator current_hash  = stack_.hashes().rbegin();
  ...
  ++current;
  ++current_hash;
  while (current != stack_.path().rend() && (**current).type == stack_.path_top()->type) {
    if (root->step)
...
void search_tree::reduce(boost::shared_ptr<player_moves> const& moves, std::size_t current_depth) {
  // Children that are meant to be transferred to the current one will temporarily be stored in transferred_children.
  // Transferring them directly would result in invalidating the iterators in the sequence that's currently being iterated
  // over. That wouldn't be nice.
  //
  // For the sake of genericity, let's not assume that some sort of container is going to be compatible with vertex -- just
  // store the children directly in a vertex. It's a slight abusal, but it ought to work.
  ...
  while (child != parent.children_end()) {
    if (child->type != parent.type) {
      ...
      boost::optional<move_path> move = moves->query(stack_.hashes_top(), stack_.state());
      if (move) {
        // A pre-existing move was found. The question now is: Is this pre-existing move at shallower depth?
        ...
        if (other_depth <= current_depth + 1) {
          ...
          // This is the shallower child -- means we're removing the other one and keeping this one.
          ...
          other->step = child->step;  // Steps don't need to be necessarily compatible.

          transferred_children.transfer_child(*other_parent, other_parent->iter_from_ptr(other));

          // If we removed all children of the other vertex, remove the other vertex as well.
          ...
          stack_.pop();  // child is going to be removed -- that could hurt when popping it frmo the stack.
          child = parent.remove_child(child);  // Remove this one as we're going to transfer the found one to the parent.
        }
      }
      else {
  ...
  // Now finish the transfer of children -- move them from transferred children really to the target vertex.
  while (transferred_children.children_count() > 0) {
    search_stack_checkpoint checkpoint(stack_);
    stack_.push(&*transferred_children.children_begin());

    parent.transfer_child(transferred_children, transferred_children.children_begin());
    store_move(moves, current_depth + 1);
  }
...
void search_tree::store_move(boost::shared_ptr<player_moves> const& moves, std::size_t current_depth) {
  assert(current_depth <= 4);
  assert(stack_.path().size() >= current_depth);

  move_path path = {{}};
  search_stack::path_sequence::const_iterator v = stack_.path().end() - current_depth;
